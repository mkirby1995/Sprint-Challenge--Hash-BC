...
* What is the runtime complexity to access an array, add or remove from the front, and add or remove from the back? O(1)
...
* What does a node do if it gets a message from another in the network with a new block? Check for consensus
* Why can't someone cheat by changing a transaction from an earlier block to give themselves coins? They would have to rehash all the transactions, very computationally intensive or impossible, depending on size of chain
